    ...
    <div class="relative max-w-7xl mx-auto px-6 py-8">
      ...
      <div class="flex items-center justify-between gap-3 mb-6 flex-wrap">
          ...
          <button
            ...
            :class="[
              ...
              activeCategory === cat
                ...
                : 'bg-white text-slate-500 hover:text-slate-700 hover:border-slate-300 border border-slate-200'
            ]"
          >
        ...
        <div class="text-xs text-slate-400">
          共 <span class="text-slate-700 font-semibold">{{ filteredTools.length }}</span> 个工具
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        <div
          ...
          :key="tool.component"
          @click="openTool(tool)"
          class="group relative glass-card p-5 cursor-pointer transition-all duration-300 hover:-translate-y-0.5 hover:border-accent/30 hover:shadow-lg hover:shadow-accent/5"
        >
          <span class="absolute right-4 top-4 text-[10px] px-2 py-0.5 rounded-full bg-slate-50 text-slate-400 border border-slate-200 font-medium">
            {{ tool.category }}
          </span>
          <div class="flex items-start gap-3">
            <div class="w-11 h-11 rounded-xl bg-gradient-to-br from-accent/15 to-accent/5 flex items-center justify-center flex-shrink-0 group-hover:from-accent/25 group-hover:to-accent/10 transition-all duration-200">
              <component :is="getIconComponent(tool.icon)" class="w-6 h-6 text-accent" />
            </div>
            <div class="flex-1 min-w-0 pr-12">
              <h3 class="font-semibold text-slate-800 mb-1 truncate group-hover:text-accent transition-colors">
                {{ tool.name }}
              </h3>
              <p class="text-xs text-slate-500 line-clamp-2 leading-relaxed min-h-[2.4em]">
                {{ tool.description }}
              </p>
            </div>
          </div>
      ...
      <div v-if="filteredTools.length === 0" class="flex flex-col items-center justify-center py-20">
        ...
        <p class="text-slate-600 text-base font-medium">未找到匹配的工具</p>
        <p class="text-slate-400 text-sm mt-1">试试其他关键词或切换分类</p>
      </div>

      <footer class="text-center mt-16 py-6 border-t border-slate-200">
        <p class="text-xs text-slate-400">Dev Tools Box · Built with Vue 3 + Tailwind CSS</p>
      </footer>
    </div>
  </div>
</template>
