          ...
          <details v-for="server in capabilities.mcpServers" :key="server.name" class="group rounded-xl border border-[#e5e6e2] bg-white p-4 open:shadow-sm" open>
            ...
            <div class="mt-3 space-y-2 border-t border-[#eff0ed] pt-3">
              <details v-for="tool in server.tools || []" :key="tool.name" class="rounded-lg border border-[#eceeea] bg-[#fafbf9] px-3 py-2">
                ...
                <div class="mt-3 space-y-2 border-t border-[#eceeea] pt-2 text-[11px] text-[#727a74]"><p class="font-medium text-[#59635d]">输入参数</p><pre class="max-h-40 overflow-auto whitespace-pre-wrap break-words rounded-md bg-[#f1f3f0] p-2 font-mono text-[10px] leading-4">{{ JSON.stringify(tool.inputSchema || tool.input_schema || {}, null, 2) }}</pre><p class="font-medium text-[#59635d]">返回信息</p><pre class="max-h-40 overflow-auto whitespace-pre-wrap break-words rounded-md bg-[#f1f3f0] p-2 font-mono text-[10px] leading-4">{{ JSON.stringify(tool.outputSchema || tool.output_schema || {}, null, 2) }}</pre></div>
              </details>
